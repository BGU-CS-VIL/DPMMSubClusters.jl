function load_and_distribute_data(distributed = false, data_path = "", data_prefix = "", load_from_file = false; all_data = nothing)
    if use_verbose
        println("Loading and distributing data:")
        @time begin
            if load_from_file
                data = load_data(data_path, prefix = data_prefix)
            else
                data = all_data
            end
            if distributed
...
function init_model(all_data = nothing)
    if random_seed != nothing
        @eval @everywhere seed!($random_seed)
    end
    if all_data == nothing
        data = load_and_distribute_data(!use_threads,data_path,data_prefix,true)
    else
        data = load_and_distribute_data(!use_threads;all_data = all_data)
    end
    total_dim = size(data,2)
    model_hyperparams = model_hyper_params(hyper_params,α,total_dim)
    labels = rand(1:initial_clusters,(size(data,2)))
    labels_subcluster = rand(1:2,(size(data,2)))
    if !use_threads
        labels = distribute(labels)
        labels_subcluster = distribute(labels_subcluster)
    end
...
function init_first_clusters!(dp_model::dp_parallel_sampling, initial_cluster_count::Int64)
    ...
    if use_threads
        ...
        update_suff_stats_posterior_threads!(dp_model.group)
    else
...
function dp_parallel(;model_params::String="",
        all_data::AbstractArray{Float64,2}= nothing,
        local_hyper_params::distribution_hyper_params = nothing,
        α_param::Float64 = 1.0,
         iters::Int64 = 100,
    ...
    if all_data != nothing
        ...
        global use_threads = threads
    else
        include(model_params)
        global use_verbose = verbose
    end
    dp_model = init_model(all_data)
...
function fit(all_data::AbstractArray{Float64,2},local_hyper_params::distribution_hyper_params,α_param::Float64;
        iters::Int64 = 100, init_clusters::Int64 = 1,seed = nothing, verbose = true, save_model = false, threads = false)
    dp_model = dp_parallel(all_data = all_data,
        local_hyper_params = local_hyper_params,
        α_param = α_param,
        iters= iters,
...
function dp_parallel(model_params::String; verbose = true, save_model = true)
    return dp_parallel(model_params = model_params, verbose = verbose, save_model = save_model)
end
...
function run_model(dp_model, first_iter, model_params="none", prev_time = 0)
    ...
    for i=first_iter:iterations
        ...
        iter_time = time()
        if use_threads
            group_step_threads(dp_model.group, no_more_splits, final, i==1)
        else
